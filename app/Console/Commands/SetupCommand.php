...
class SetupCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Install MetisPOS from the command line.';
    ...
    public function handle()
    {
        ...
        if ( Helper::installed() ) {
            return $this->error( __( 'MetisPOS is already installed.' ) );
        }
        ...
        if ( in_array( strtolower( $answer ), [ 'y', 'yes' ] ) || $this->requireConfirmation === false ) {
            ...
            return $this->info( 'Thank you, MetisPOS has been successfully installed.' );
        } else {
    ...
    private function setupLanguage()
    {
        while ( empty( $this->language ) ) {
            $langIndex = $this->choice( __( 'In which language would you like to install MetisPOS ?' ), array_values( config( 'nexopos.languages' ) ) );
